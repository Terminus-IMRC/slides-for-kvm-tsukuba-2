...
\begin{frame}\frametitle{謝罪}
\begin{itemize}
\item 「$n$次魔方陣全解問題」は整数問題で、かつベクトル演算を含まない
\item $\longrightarrow$XeonPhiに不適だと初めから分かっていた
\end{itemize}
\end{frame}
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.9\textwidth]{result.eps}
\end{figure}
\end{frame}

\begin{frame}\frametitle{考察}
\begin{itemize}
\item 整数問題においては、pthreadではなくOpenMPを使用した方がプログラミングコストも低く、XeonPhiの性能が出る
\item XeonPhiは整数問題には向いていないかも
\end{itemize}
\end{frame}

\begin{frame}\frametitle{宣伝}
\centering
10月21日$\sim$22日 \\
学際計算科
学による新たな知の発見・統合・創出シンポジウム
\end{frame}
\end{document}
